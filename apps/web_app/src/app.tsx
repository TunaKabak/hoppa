...
export default function App({ initialTab = 'user' }: { initialTab?: string }) {
    ...
    const [merchantCategory, setMerchantCategory] = useState('partners_merchant_form_cat1');
    const [showMerchantCategoryDropdown, setShowMerchantCategoryDropdown] = useState(false);
                                        ...
                                        {partnerSubmitted ? (
                                            ...
                                            <form onSubmit={(e) => { e.preventDefault(); setPartnerSubmitted(true); }} className="space-y-4">
                                                ...
                                                <div className="space-y-1.5 relative">
                                                    <label className="text-xs font-bold text-slate-500 block">{t('partners_merchant_form_cat')}</label>
                                                    <div className="relative">
                                                        <button
                                                            type="button"
                                                            onClick={() => setShowMerchantCategoryDropdown(!showMerchantCategoryDropdown)}
                                                            className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-600 transition-all text-left flex items-center justify-between font-medium text-slate-700 hover:bg-slate-50/50"
                                                        >
                                                            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400">
                                                                <Store size={18} />
                                                            </div>
                                                            <span className={merchantCategory ? 'text-slate-800 font-semibold' : 'text-slate-400'}>
                                                                {merchantCategory ? t(merchantCategory as any) : 'Kategori seçiniz...'}
                                                            </span>
                                                            <ChevronDown size={18} className={`text-slate-400 transition-transform ${showMerchantCategoryDropdown ? 'rotate-180' : ''}`} />
                                                        </button>
                                                            ...
                                                            <>
                                                                ...
                                                                <div className="absolute left-0 right-0 top-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-2xl z-50 py-2 max-h-60 overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-200">
                                                                    {[
                                                                        ...
                                                                        'partners_merchant_form_cat3',
                                                                        'partners_merchant_form_cat4',
                                                                        'partners_merchant_form_cat5'
                                                                    ].map(catKey => (
                                                                        <button
                                                                            key={catKey}
                                                                            type="button"
                                                                            onClick={() => {
                                                                                setMerchantCategory(catKey);
                                                                                setShowMerchantCategoryDropdown(false);
                                                                            }}
                                                                            className={`w-full px-5 py-3 text-left text-sm transition-colors flex items-center justify-between ${
                                                                                merchantCategory === catKey 
                                                                                    ? 'bg-emerald-50 text-emerald-700 font-bold' 
                                                                                    : 'text-slate-600 hover:bg-slate-50'
                                                                            }`}
                                                                        >
                                                                            <span>{t(catKey as any)}</span>
                                                                            {merchantCategory === catKey && <Check size={16} className="text-emerald-600" />}
                                                                        </button>
                                                                    ))}
                                                                </div>
                                                            </>
