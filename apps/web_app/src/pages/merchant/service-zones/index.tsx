  ...
  return (
    ...
    >
      <div className="w-full max-w-[1700px] mx-auto space-y-6">
        ...
        {/* Main Workspace Layout (Unified Single Map Instance for Instant Zero-Flicker Layout Switching) */}
        <div className="space-y-4">
          {/* Quick Floating Action Bar when Canvas (Large Map) Mode is Active */}
          {layoutMode === 'canvas' && (
            <div className={`p-4 rounded-2xl border flex flex-col md:flex-row md:items-center justify-between gap-3 animate-in fade-in-0 duration-200 ${
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            {/* Left Column: Zones List & Editor (Visible only in Split Mode) */}
            {layoutMode === 'split' && (
                      ...
                      <div className="grid grid-cols-2 gap-2.5">
                        <div>
                          <HoppaSelect
                            ...
                            onChange={(val) => setNewZoneDistrict(val)}
                            options={[
                              { value: 'Lefkoşa', label: 'Lefkoşa', badge: 'Başkent' },
                              { value: 'Girne', label: 'Girne', badge: 'Turizm' },
                              { value: 'Gazimağusa', label: 'Gazimağusa', badge: 'Liman' },
                              { value: 'Güzelyurt', label: 'Güzelyurt', badge: 'Narenciye' },
                              { value: 'İskele', label: 'İskele', badge: 'Sahil' },
                              { value: 'Lefke', label: 'Lefke', badge: 'Üniversite' },
                              { value: 'KKTC Genel', label: 'KKTC Genel', badge: 'Tüm Ada' },
                            ]}
                            size="sm"
                            variant="contrast"
                          />
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                <div>
                                  <HoppaSelect
                                    label="İlçe"
                                    value={zone.district}
                                    onChange={(val) => handleUpdateZoneField(zone.id, { district: val })}
                                    options={[
                                      { value: 'Lefkoşa', label: 'Lefkoşa', badge: 'Başkent' },
                                      { value: 'Girne', label: 'Girne', badge: 'Turizm' },
                                      { value: 'Gazimağusa', label: 'Gazimağusa', badge: 'Liman' },
            ...
            {/* Right / Main Column: Large Interactive Leaflet Map (Unified Container) */}
            <div className={layoutMode === 'split' ? 'lg:col-span-8' : 'lg:col-span-12'}>
              <div className={`p-4 rounded-3xl border transition-all ${
                ...
                <KktcServiceZoneDrawerMap
                  ...
                  heightClass={layoutMode === 'canvas' ? 'h-[820px]' : 'h-[760px]'}
                />
              </div>
            </div>
          </div>
