...
router.post("/register", async (req, res) => {
    try {
        const { email, password, businessName, msNumber, taxNumber, phone, district, fullAddress, } = req.body;
        // 1. Validasyon
        if (!email ||
            !password ||
            !businessName ||
 ...
 */
router.post("/submit-revision", async (req, res) => {
    try {
        const { uid, businessName, msNumber, taxNumber, phone, district, fullAddress, } = req.body;
        ...
        const userData = userDoc.data();
        if ((userData === null || userData === void 0 ? void 0 : userData.status) !== "revision_requested") {
            return res.status(400).json({
                success: false,
        ...
        await userRef.update({
            ...
            fullAddress,
            status: "pending",
            revisionMessage: firestore_1.FieldValue.delete(), // Mesajı kaldırıyoruz
        });
        return res.status(200).json({
            success: true,
